...
void decodeData(unsigned int i, unsigned int j, int charLength, cv::Mat &src, 
    ...
    for (int k = 0; k < charLength; k++) {
        for (int h = 0; h < 8; h++) {
            ...
            color = randNum() % 3;
            word = (h == 0) ? (src.at<cv::Vec3b>(i, j)[color] & (unsigned)0x1) : (word << 1) | (src.at<cv::Vec3b>(i, j)[color] & 0x1);
...
void encodeData(unsigned int startY, unsigned int startX, int phraseLength, cv::Mat &src, std::string *wordList,
                std::mt19937::result_type seed, unsigned long skipSeed, int randMin, int randMax) {
